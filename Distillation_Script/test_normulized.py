...
Implements: adaptive loss weighting, learning rate scheduling, mixed precision
"""
...
class DistillationConfig:
    ...
    use_dynamic_lambda: bool = True  # Adaptive loss weighting
    use_lr_scheduling: bool = True   # Learning rate scheduling
    warmup_steps: int = 500          # Warmup iterations
    use_mixed_precision: bool = True # FP16 training
    grad_accumulation_steps: int = 2 # Gradient accumulation
    use_dynamic_temp: bool = True    # Adaptive temperature
...
class HybridLoss(nn.Module):
    """Hybrid Loss with dynamic temperature"""
    ...
    def retrieval_loss(self, query_emb, passage_emb):
        """Contrastive retrieval loss (InfoNCE-style) with normalized embeddings"""
        ...
        # Cosine similarity is already computed correctly with normalized embeddings
        sim_matrix = torch.mm(query_norm, passage_norm.t()) / self.temperature
...
def create_student_model(student_name: str) -> SentenceTransformer:
    """Create student model with layer normalization"""
    ...
    # Add layer normalization for stable embeddings
    norm = models.Normalize()
    student = SentenceTransformer(modules=[transformer, pooling, norm])
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for batch_idx, (queries, passages) in enumerate(pbar):
                ...
                with torch.no_grad():
                    ...
                    # Normalize teacher embeddings for consistency
...
def distill_ict_model(
    ...
    logger.info("\n" + "="*80)
    logger.info(f"ICT-{variant.upper()} KNOWLEDGE DISTILLATION (DYNAMIC LEARNING)")
    logger.info("="*80 + "\n")
    ...
    config = DistillationConfig(
        ...
        model_name=f"ict_{variant}_distilled_dynamic",
    ...
    logger.info("\n" + "="*80)
    logger.info("✓ DYNAMIC DISTILLATION COMPLETE")
    logger.info("="*80 + "\n")
...
if __name__ == "__main__":
    student_model = distill_ict_model(
        ...
        lambda_weight=0.5,
        temperature=1.0,
        use_dynamic_lambda=True,        # Enable dynamic λ
